...
Status: **Implemented (Prompt #5 baseline) — Drift/SQLite `media_items` index,
transactional batch upserts, persist-then-ACK orchestration, migration scaffold.**
...
- Idempotent: N scans of the same file ⇒ exactly one row, monotonically newer
  metadata. **No row deletion is ever inferred from a scan's omission** — a partial
  view (`accessScope == partial`) must not erase items; deletion reconciliation is a
...
- `deleteByStableKey(…)` → `bool`; `clearAll()` (explicit app-level reset only)
...
- `schemaVersion = 1`. A clean v1 install runs `onCreate`; `onUpgrade` is empty by
  design (no v0→v1 upgrade path exists yet).
- The authoritative schema snapshot is committed at
  `drift_schemas/drift_schema_v1.json` (generated via
  `dart run drift_dev schema dump lib/core/database/app_database.dart drift_schemas/`).
  Future prompts add a snapshot per new version and a real `MigrationStrategy`
  before any v2 migration lands.
- The migration test asserts `PRAGMA user_version = 1`, the full `media_items`
  column set, and data surviving close/reopen (file-backed DB).

## 10. Out of Scope (deferred, do not implement)

volumes/index_state/index_errors/saf_grants tables, document (SAF) ingestion,
deletion reconciliation, incremental sync cursors, drift-watcher-based polling,
search/FTS, OCR/text extraction, thumbnails, embeddings, scheduled/WorkManager
